...
class CustomLegendControl extends L.Control {
  ...
  onAdd(map: L.Map): HTMLElement {
    ...
    labels.push(
      '<div class="title" id="legend-title" role="heading" aria-level="2">Population Density</div>'
    );
...
export default function Map({
  ...
  const style = useMemo(() => {
    ...
    return (
      feature: GeoJSON.Feature<GeoJSON.Geometry, ProvinceProperties> | undefined
    ) => {
      if (!feature) return {};
      return {
  ...
  const highlightFeature = useCallback(
    (e: L.LeafletEvent) => {
      ...
      if (layer.feature && layer.feature.properties) {
        ...
        const announcement = `${props.name}: ${
          props.p ? props.p.toLocaleString() : "Unknown"
  ...
  const onEachFeature = useCallback(
    (
      ...
      // Enhanced touch support for mobile devices with better feedback
      if (leafletLayer instanceof L.Path) {
        const element = leafletLayer.getElement();
        if (element) {
          // Add a class to increase touch target size
          element.classList.add("touch-target");

          // Track touch start position for better gesture detection
          let touchStartX = 0;
          let touchStartY = 0;
          let touchStartTime = 0;

          element.addEventListener("touchstart", (e: Event) => {
            const domEvent = e as TouchEvent;
            domEvent.preventDefault();
            // Store touch start position and time
            if (domEvent.touches.length > 0) {
              touchStartX = domEvent.touches[0].clientX;
              touchStartY = domEvent.touches[0].clientY;
            ...
            const typedLayer = leafletLayer as GeoJSONLayer;
            if (typedLayer.feature && typedLayer.feature.properties) {
              const props = typedLayer.feature.properties;
              announceToScreenReader(`Selected province: ${props.name}`);
            }
          });

          element.addEventListener("touchmove", (e: Event) => {
            const domEvent = e as TouchEvent;
            // If touch moves significantly, it's likely a map pan gesture
            if (domEvent.touches.length > 0) {
              const touchMoveX = domEvent.touches[0].clientX;
              const touchMoveY = domEvent.touches[0].clientY;
          ...
          element.addEventListener("touchend", (e: Event) => {
            ...
            // If touch was short (tap), zoom to the province
            if (touchDuration < 300) {
              zoomToFeature({ target: leafletLayer } as L.LeafletEvent);
            }

            // Keep the highlight for a while for better user experience
            setTimeout(() => {
            ...
            }, 2000); // Longer timeout for better visibility on mobile
          });
        }
      }
    },
  ...
  useEffect(() => {
    ...
    if (!document.body.contains(containerRef.current)) {
      console.log(
        "Container is not in the DOM yet, delaying initialization..."
      );
      const checkInterval = setInterval(() => {
        if (
          ...
          console.log(
            "Container is now in the DOM, proceeding with initialization"
          );
    ...
    try {
      ...
      // Share map instance with parent component if callback provided
      if (onMapInstance) {
        onMapInstance(map);
      }
  ...
  useEffect(() => {
    ...
    mapContainer.setAttribute(
      "aria-label",
      "Interactive map of Thailand population density. Use arrow keys to pan, + and - to zoom."
    );
  ...
  useEffect(() => {
    ...
    touchInstructions.textContent =
      "Map loaded. Use two fingers to pan, pinch to zoom, and tap on provinces to see details.";
  ...
  useEffect(() => {
    ...
    console.log(
      `GeoJSON data size: ${JSON.stringify(geoJSONData).length} bytes`
    );
    ...
    if (mapRef.current) {
      ...
      mapRef.current.getRenderer = function () {
        return L.canvas({ padding: 0.5, tolerance: 10 });
      };
    }
